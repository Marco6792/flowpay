...
import { Balance, BalanceResponse } from './providers/provider.interface.ts';
...
export class BalanceService {
   ...
   */
  static async getAggregatedBalance(userId: string): Promise<BalanceAggregationResponse> {
    try {
      logger.info({ userId }, 'Starting balance aggregation');
      ...
      // Get provider balances in parallel
      const providerResults = await Promise.allSettled([
        this.getProviderBalance('mtn'),
        this.getProviderBalance('orange')
      ]);
   ...
   */
  private static async getProviderBalance(providerName: string): Promise<BalanceResponse> {
    try {
