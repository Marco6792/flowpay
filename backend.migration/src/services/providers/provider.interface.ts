export interface PaymentProvider {
   ...
   */
  createInvoice(params: InvoiceRequest): Promise<InvoiceResponse>;

  /**
   * Get invoice status
   */
  getInvoiceStatus(referenceId: string): Promise<InvoiceStatus>;

  /**
   * Cancel an invoice
   */
  cancelInvoice(referenceId: string): Promise<{ success: boolean; message?: string }>;

  /**
   * Create a cash transfer (remittance)
   */
  cashTransfer(params: CashTransferRequest): Promise<CashTransferResponse>;

  /**
   * Get cash transfer status
   */
  getCashTransferStatus(referenceId: string): Promise<CashTransferStatus>;

  /**
   * Get approved pre-approvals for an account holder
   */
  getApprovedPreApprovals(accountHolderIdType: string, accountHolderId: string): Promise<ApprovedPreApproval[]>;

  /**
   * Get account balance in a specific currency
   */
  getBalanceByCurrency(currency: string): Promise<BalanceByCurrencyResponse>;

  // V2 API Methods

  /**
   * Check payment status using v2.0 API
   */
  checkStatusV2(referenceId: string): Promise<PaymentStatus>;

  /**
   * Request withdrawal using v2.0 API
   */
  requestWithdrawV2(params: WithdrawRequest): Promise<WithdrawResponse>;

  /**
   * Deposit funds using v2.0 API
   */
  depositV2(params: DepositRequest): Promise<DepositResponse>;

  /**
   * Process refund using v2.0 API
   */
  refundV2(transactionId: string, amount?: number, reason?: string): Promise<RefundResponse>;
}
...
export interface CashTransferStatus {
  transferId: string;
  referenceId: string;
  status: string;
  amount: number;
...
export interface ApprovedPreApproval {
  referenceId: string;
  status: string;
  payer: {
    partyIdType: string;
...
export interface BalanceByCurrencyResponse {
