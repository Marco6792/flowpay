...
export class MTNMobileMoneyProvider implements PaymentProvider {
  ...
  constructor(environment?: 'sandbox' | 'production') {
    this.environment = environment || (env.MTN_TARGET_ENVIRONMENT as 'sandbox' | 'production') || "sandbox";
    ...
    if (!this.apiKey || !this.apiSecret || !this.collectionSubscriptionKey) {
      logger.warn(
        `MTN Mobile Money provider (${this.environment}) not fully configured. Missing API credentials.`
      );
    }

    logger.info(`MTN Mobile Money provider initialized in ${this.environment} mode → ${this.baseUrl}`);
  }
  ...
  // ========== Part B: New MTN API Methods ==========

  /**
   * Create an invoice for deferred payment (Collection API v2.0)
   */
  async createInvoice(params: any): Promise<any> {
    try {
      ...
      const requestBody = {
        ...
        validityDuration: params.validityDuration || 3600,
        intendedPayer: params.intendedPayer,
        payee: params.payee,
        description: params.description,
      };

      const response = await fetch(`${this.baseUrl}/collection/v2_0/invoice`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'X-Reference-Id': referenceId,
      ...
      if (response.ok || response.status === 202) {
        logger.info({ referenceId, externalId: params.externalId }, 'Invoice created successfully');
        return {
          success: true,
          invoiceId: params.invoiceId || referenceId,
          referenceId,
          status: 'PENDING',
          externalId: params.externalId,
      ...
      const errorText = await response.text();
      logger.error({ status: response.status, error: errorText }, 'Failed to create invoice');
      return {
        success: false,
        invoiceId: params.invoiceId || referenceId,
        referenceId,
        status: 'FAILED',
        externalId: params.externalId,
        message: `Invoice creation failed: ${errorText}`,
      };
    } catch (error: any) {
  ...
  async getInvoiceStatus(referenceId: string): Promise<any> {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/collection/v2_0/invoice/${referenceId}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'X-Target-Environment': this.targetEnvironment,
      ...
      if (response.ok) {
        ...
        return {
          ...
          intendedPayer: data.intendedPayer,
      ...
      logger.error({ error, referenceId }, 'Error getting invoice status');
      throw error;
    }
  }
   ...
   * Cancel an invoice
   */
  async cancelInvoice(referenceId: string): Promise<{ success: boolean; message?: string }> {
    try {
      const accessToken = await this.getAccessToken();
      ...
      if (response.ok || response.status === 202 || response.status === 204) {
        logger.info({ referenceId }, 'Invoice cancelled successfully');
        return { success: true, message: 'Invoice cancelled successfully' };
      }

      const errorText = await response.text();
      logger.error({ status: response.status, error: errorText }, 'Failed to cancel invoice');
      return { success: false, message: `Failed to cancel invoice: ${errorText}` };
    } catch (error: any) {
      logger.error({ error, referenceId }, 'Error cancelling invoice');
      return { success: false, message: error.message };
    }
  }

  /**
   * Create a cash transfer (Remittance API v2.0)
   */
  async cashTransfer(params: any): Promise<any> {
    try {
      const accessToken = await this.getAccessToken();
      const referenceId = crypto.randomUUID();
      ...
      const response = await fetch(`${this.baseUrl}/remittance/v2_0/cashtransfer`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'X-Reference-Id': referenceId,
      ...
      if (response.ok || response.status === 202) {
        logger.info({ referenceId, externalId: params.externalId }, 'Cash transfer initiated');
        return {
          success: true,
          transferId: params.transferId || referenceId,
          referenceId,
          status: 'PENDING',
      ...
      const errorText = await response.text();
      logger.error({ status: response.status, error: errorText }, 'Failed to create cash transfer');
      return {
        success: false,
        transferId: params.transferId || referenceId,
        referenceId,
        status: 'FAILED',
        message: `Cash transfer failed: ${errorText}`,
      };
    } catch (error: any) {
      logger.error({ error }, 'Error creating cash transfer');
      throw error;
    }
  }
   ...
   */
  async getCashTransferStatus(referenceId: string): Promise<any> {
    try {
      const accessToken = await this.getAccessToken();

      const response = await fetch(`${this.baseUrl}/remittance/v2_0/cashtransfer/${referenceId}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'X-Target-Environment': this.targetEnvironment,
      ...
      throw new Error(`Failed to get cash transfer status: ${response.status}`);
    } catch (error: any) {
      logger.error({ error, referenceId }, 'Error getting cash transfer status');
      throw error;
    }
  }

  /**
   * Get approved pre-approvals for an account holder
   */
  async getApprovedPreApprovals(accountHolderIdType: string, accountHolderId: string): Promise<any[]> {
    try {
      const accessToken = await this.getAccessToken();

      const response = await fetch(
        `${this.baseUrl}/collection/v1_0/preapprovals/${accountHolderIdType}/${accountHolderId}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      if (response.ok) {
        const data = await response.json();
        const preApprovals = Array.isArray(data) ? data : [];
        return preApprovals.map((pa: any) => ({
      ...
      logger.warn(
        { status: response.status, accountHolderIdType, accountHolderId },
        'Failed to get approved pre-approvals'
      );
      return [];
    } catch (error: any) {
      logger.error({ error, accountHolderIdType, accountHolderId }, 'Error getting approved pre-approvals');
      return [];
    }
  }

  /**
   * Request withdrawal using MTN Collection v2.0 API
   */
  async requestWithdrawV2(params: WithdrawRequest): Promise<WithdrawResponse> {
    try {
      const accessToken = await this.getAccessToken();
      const referenceId = crypto.randomUUID();
      ...
      const requestBody = {
        payeeNote: params.description || "FlowPay withdrawal v2",
        externalId: params.withdrawId,
        amount: params.amount.toString(),
        currency: currency,
        payer: {
        ...
        payerMessage: params.payerMessage || "Withdrawal request v2",
      };

      logger.info(
        { withdrawId: params.withdrawId, referenceId, amount: params.amount },
        "Initiating MTN v2 withdrawal request"
      );
      ...
      logger.info(
        { referenceId, httpStatus: response.status, responseData },
        "MTN v2 withdrawal response"
      );
      ...
      if (response.status === 202) {
        return {
          ...
          message: "Withdrawal initiated successfully (v2)",
          timestamp: new Date(),
          httpStatus: response.status,
          httpStatusText: response.statusText,
          responseHeaders: Object.fromEntries(response.headers.entries()),
        ...
        return {
          ...
          referenceId: referenceId,
          status: WithdrawStatusEnum.FAILED,
          message: responseData?.message || `MTN v2 withdrawal failed: ${response.status}`,
          timestamp: new Date(),
          httpStatus: response.status,
          httpStatusText: response.statusText,
          responseHeaders: Object.fromEntries(response.headers.entries()),
      ...
      logger.error({ error, withdrawId: params.withdrawId }, "Error processing MTN v2 withdrawal");
      return {
        success: false,
        withdrawId: params.withdrawId,
        providerWithdrawId: "",
        referenceId: "",
        status: WithdrawStatusEnum.FAILED,
        message: `V2 withdrawal request failed: ${error.message}`,
        timestamp: new Date(),
      };
    }
  }
  ...
  /**
   * Deposit funds using MTN Disbursement v2.0 API
   */
  async depositV2(params: DepositRequest): Promise<DepositResponse> {
    try {
      ...
      const depositRequest = {
        ...
        payerMessage: params.description || "Deposit to account (v2)",
        payeeNote: "Deposit from FlowPay (v2)",
      };
      ...
      if (
        (this.targetEnvironment === "sandbox" &&
          (this.callbackUrl.includes("webhook.site") || this.callbackUrl.includes("ngrok"))) ||
        this.targetEnvironment === "production"
      ) {
        headers["X-Callback-Url"] = this.callbackUrl;
      }
      ...
      logger.info(
        { depositReferenceId, request: depositRequest },
        "Sending MTN v2 deposit request"
      );
      ...
      logger.info(
        { depositReferenceId, status: response.status },
        "MTN v2 deposit response"
      );
      ...
      if (response.status === 202) {
        return {
          ...
          providerDepositId: depositReferenceId,
          status: DepositStatusEnum.PENDING,
          message: "Deposit initiated successfully (v2)",
          timestamp: new Date(),
          httpStatus: response.status,
          httpStatusText: response.statusText,
          responseHeaders: Object.fromEntries(response.headers.entries()),
        ...
        const error = await response.text();
        logger.error({ statusCode: response.status, error }, "MTN v2 deposit failed");
        return {
          success: false,
          depositId: params.depositId,
          providerDepositId: depositReferenceId,
          status: DepositStatusEnum.FAILED,
          message: `V2 deposit initiation failed: ${error}`,
          timestamp: new Date(),
          httpStatus: response.status,
          httpStatusText: response.statusText,
          responseHeaders: Object.fromEntries(response.headers.entries()),
      ...
      logger.error({ error, depositId: params.depositId }, "Error processing MTN v2 deposit");
      return {
        success: false,
        depositId: params.depositId,
        providerDepositId: "",
        status: DepositStatusEnum.FAILED,
        message: `V2 deposit failed: ${error.message}`,
        timestamp: new Date(),
      };
    }
  }
  ...
  /**
   * Process refund using MTN Disbursement v2.0 API explicitly
   */
  async refundV2(transactionId: string, amount?: number, reason?: string): Promise<RefundResponse> {
    // The existing refund() method already tries v2.0 first, then falls back to v1.0
    // This method explicitly uses only v2.0
    try {
      const accessToken = await this.getAccessToken("disbursement");
      ...
      const refundRequest = {
        ...
        payerMessage: reason || "Refund v2",
        payeeNote: "FlowPay refund v2",
        referenceIdToRefund: transactionId,
      };
      ...
      logger.info({ refundReferenceId, transactionId }, "Sending MTN v2 refund request");

      const response = await fetch(`${this.baseUrl}/disbursement/v2_0/refund`, {
        method: "POST",
        headers,
        body: JSON.stringify(refundRequest),
      });
      ...
      logger.info(
        { refundReferenceId, status: response.status },
        "MTN v2 refund response"
      );
      ...
      if (response.status === 202) {
        ...
        return {
          ...
          amount: refundAmount,
          status: refundStatus.status,
          message: "Refund initiated successfully (v2)",
          financialTransactionId: (refundStatus as any).financialTransactionId,
        };
      } else {
        const error = await response.text();
        logger.error({ refundReferenceId, statusCode: response.status, error }, "MTN v2 refund failed");
        return {
          success: false,
          refundId: refundReferenceId,
          amount: refundAmount,
          status: RefundStatusEnum.FAILED,
          message: `V2 refund failed: ${error}`,
        };
      }
    } catch (error: any) {
      logger.error({ error, transactionId }, "Error processing MTN v2 refund");
      return {
        success: false,
        refundId: "",
        amount: amount || 0,
        status: RefundStatusEnum.FAILED,
        message: `V2 refund failed: ${error.message}`,
      };
    }
  }

  async getBalanceByCurrency(currency: string): Promise<any> {
    try {
      const accessToken = await this.getAccessToken();

      const response = await fetch(`${this.baseUrl}/collection/v1_0/account/balance/${currency}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'X-Target-Environment': this.targetEnvironment,
      ...
      throw new Error(`Failed to get balance for currency ${currency}: ${response.status}`);
    } catch (error: any) {
      logger.error({ error, currency }, 'Error getting balance by currency');
      throw error;
    }
  }
}
