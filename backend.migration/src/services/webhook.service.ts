...
export class WebhookService {
   ...
   */
  static async queueWebhook(transactionId: string, event: WebhookPayload['event'], webhookUrl: string, merchantSecret?: string): Promise<void> {
    try {
      ...
      const delivery = await prisma.webhookDelivery.create({
        data: {
          ...
          providerSignature: this.generateWebhookSignature(payload, merchantSecret),
        } as any,
      });
  ...
  static async processWebhook(deliveryId: string): Promise<void> {
    try {
      ...
      // Generate signature for webhook (use the stored signature from creation which uses the merchant's secret)
      const signature = (delivery as any).providerSignature || this.generateWebhookSignature((delivery as any).payload);
   ...
   */
  private static generateWebhookSignature(payload: any, merchantSecret?: string): string {
    const secret = merchantSecret || env.WEBHOOK_SECRET || 'default_webhook_secret';
